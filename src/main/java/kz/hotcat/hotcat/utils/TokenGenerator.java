...
public class TokenGenerator {
    ...
    public TokenDTO createTokens(Authentication authentication) {
        if(!(authentication.getPrincipal() instanceof AppUser appUser)) {
            throw new BadCredentialsException("Principal is not AppUser type");
        }

        TokenDTO tokenDTO = new TokenDTO();
        tokenDTO.setUserId(appUser.getId().toString());
        ...
        if(authentication.getCredentials() instanceof Jwt jwt) {
            ...
            Instant now = Instant.now();
            Duration duration = Duration.between(now, tokenExpiresAt);
